...
<div class="gradient-bg text-white rounded-lg p-8 mb-8">
    <div class="text-center">
        <h1 class="text-4xl font-bold mb-4"><?= htmlspecialchars($title ?? 'Welcome to Brick Framework') ?></h1>
        <p class="text-xl opacity-90"><?= htmlspecialchars($message ?? 'A modern PHP framework for rapid development') ?></p>
        <p class="mt-4 opacity-75">Version <?= htmlspecialchars($version ?? '1.0.0') ?> running in <?= htmlspecialchars($environment ?? 'development') ?> mode</p>
    </div>
</div>
        ...
        <div class="flex items-center mb-4">
            <div class="bg-green-100 p-3 rounded-full">
                <svg class="w-6 h-6 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l3 3-3 3m5 0h3M5 20h14a2 2 0 002-2V6a2 2 0 00-2-2H5a2 2 0 002 2v12a2 2 0 002 2z"></path>
                </svg>
            </div>
...
// Include layout
$title = $title ?? 'Home - Brick Framework';
$app_name = 'Brick Framework';
$app_env = $environment ?? 'development';
include __DIR__ . '/../layouts/simple-app.php';
?>